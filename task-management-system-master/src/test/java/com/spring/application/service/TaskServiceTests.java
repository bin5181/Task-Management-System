...
class TaskServiceTest {
    ...
    void testCreateTaskSuccess() {
        ...
        when(userRepository.existsById(assignedUserId)).thenReturn(true);
        ...
        TaskDto createdTask = taskService.createTask(taskDto, assignedUserId);

        assertEquals(TaskStatus.PENDING, createdTask.getStatus()); // Default status if not set
        assertEquals(assignedUserId, task.getAssignedTo());
        assertNotNull(createdTask);
        verify(userRepository, times(1)).existsById(assignedUserId);
    ...
    void testCreateTaskUserNotFound() {
        ...
        when(userRepository.existsById(assignedUserId)).thenReturn(false);

        UserNotFoundException exception = assertThrows(UserNotFoundException.class, () -> {
            taskService.createTask(taskDto, assignedUserId);
        });

        assertEquals("User not found with this user id", exception.getMessage());
        verify(taskRepository, never()).save(any(Task.class)); // Ensure task is not saved
    }
